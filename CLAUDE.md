...
`.claude/settings.json` (checked in) enables the required plugins and runs `.claude/startup-check.sh` at every SessionStart. It verifies plugins (caveman, ponytail, superpowers, frontend-design, playwright, vercel), MCP health (playwright, supabase, vercel + claude.ai Gmail/Calendar/Drive), the `le-nouette-*` personas, `core.hooksPath=.githooks`, the Playwright kit and Chrome, and `app/.env.local`. The result is injected into context. **First reply of a session: report any `[RED]` item to the user in one line before starting work** (claude.ai connectors needing auth are only `[warn]`). The script cannot install or authorize anything — fix via `/plugin`, claude.ai connector settings, or the command it prints. Keep the script's `PLUGINS`/`MCPS` lists in sync when the toolset changes (docs persona rule applies).
