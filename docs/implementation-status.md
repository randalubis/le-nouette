...
| Product photography (real images deployed) | ✅ BUILT | `app/public/le-nouette/milieu.png`, `app/public/le-nouette/grande.png` (hero.png), referenced in `storefront.tsx:18`. Images optimized via next.config.ts `images: { qualities: [60, 75] }` and deployed with explicit `quality={60}` and `sizes` attributes for responsive sizing; Vercel build verified. |
