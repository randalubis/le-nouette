...
| Startup checklist (SessionStart hook) | ✅ BUILT | `.claude/startup-check.sh` (verifies plugins, MCP servers, agents, git hooks, local config); `.claude/settings.json` wires it as SessionStart hook. Warns on missing setup without blocking session. See `docs/technical/dev-workflow.md` §3. |
